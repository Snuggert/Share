...
static builtin eigen[] = {{do_exit, "exit"},
		{do_cd, "cd"},
		{do_source, "source"},
		{do_source, "."},
		{NULL, ""}};

/* The function executeCommand will take (a part of) the commanline
...
executeCommand(unsigned char *commandStr)
{
	...
	/* Here you should be about ready to call execv. Do not forget to
	   handle the situation where execv does return */
...
parseCommand(unsigned char *commandStr)
{
	...
	/* If there is no '|' on the command-line, we fork and execute the first
	   ...
	   then call fork(), letting the child know that it should connect its output to the
		pipe, and what program it should execute with what parameters.
